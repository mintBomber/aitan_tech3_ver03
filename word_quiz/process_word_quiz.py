...
def decrease_search_count(word):
    ...
    for file_name in ['word_db.csv', 'paper_db.csv']:
        ...
        if word in df['Word'].values:
            ...
            df.loc[df['Search Count'] < 0, 'Search Count'] = 0
...
def _extraction_from_category(df_word, df_paper, n_word_test, quiz_category):
    ...
    try:
        # 共通するカラム抽出
        common_columns = ['Word', 'Meaning', 'Category', 'Search Count', 'Add Date', 'Importance']
       ...
       # 'Search Count'の値を10でcut編集
        df_filtered['Search Count'] = df_filtered['Search Count'].apply(lambda x: x if x <= 9 else 10)

        # 'Search Count'カラムと'Importance'カラムの値を足し合わせた新しいカラムを作成
        df_filtered['Total Score'] = df_filtered['Search Count'] + df_filtered['Importance']
...
def _extraction_from_thesis(df, n_word_test, quiz_category):
    """論文データからテストの単語抽出"""
    try:
        # 'Category'カラムがquiz_categoryと等しい行のみを抽出
...
def process_csv(file_name, n_word_test, quiz_mode, quiz_category):
    ...
    try:
        ...
        if quiz_mode == '基本単語帳もーど':
            ...
            df = _load_csv(file_name)
            top_words = _extraction_from_thesis(df, n_word_test, quiz_category)
